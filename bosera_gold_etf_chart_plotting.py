...
def plot_bosera_gold_etf_nav(excel_file: str,
                             sheet_name: str = 0,
                             years: int = 3,
                             output_html: Optional[str] = None):
    """
    绘制博时黄金ETF单位净值折线图（HTML交互版）

    Args:
        excel_file: Excel 文件路径
        sheet_name: Sheet 名称或索引（默认第 1 个）
        years: 显示最近 N 年的数据
        output_html: 输出 HTML 路径；若为 None，则与 Excel 同目录，命名为“博时黄金ETF_近N年单位净值走势图.html”
    """
    print(f"正在读取Excel文件: {excel_file}")
    print(f"Sheet: {sheet_name}")
    df = pd.read_excel(excel_file, sheet_name=sheet_name, engine="openpyxl")

    required_cols = ["日期", "单位净值(元)"]
    ...
    fig.update_layout(
        title={
            "text": f"博时黄金ETF - 近{years}年单位净值走势图",
            "x": 0.5,
            "xanchor": "center",
        ...
        yaxis=dict(
            title=dict(text="单位净值(元)", font=dict(size=14)),
            showgrid=True,
            gridcolor="rgba(128, 128, 128, 0.2)"
        ),
        hovermode="x unified",
    ...
    if output_html is None:
        ...
        if not excel_dir:
            excel_dir = os.getcwd()
        output_html = os.path.join(excel_dir, f"博时黄金ETF_近{years}年单位净值走势图.html")
    ...
    config = {
        ...
        "toImageButtonOptions": {
            "format": "png",
            "filename": f"博时黄金ETF_近{years}年单位净值走势图",
            "height": 700,
            "width": 1400,
            "scale": 1
        }
