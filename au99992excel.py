...
def get_recent_gold_data_from_date(start_date_obj):
    """
    从指定日期开始获取黄金数据直到今天
    :param start_date_obj: 开始日期对象
    :return: 包含黄金数据的DataFrame
    ...
    # 如果开始日期在2023年及以前，且今天是2024年及以后
    needs_historical_data = start_date_obj.year <= 2023
    needs_current_data = today.year >= 2024

    if needs_historical_data and start_date_obj <= date(2023, 12, 31):
        # 获取历史数据（2023年及以前的部分）
        hist_start_date = start_date_obj
        hist_end_date = min(datetime(2023, 12, 31).date(), today)
        if hist_end_date >= hist_start_date:
            print(f"获取 {hist_start_date} 至 {hist_end_date} 历史数据...")
            historical_df = get_historical_gold_data(hist_start_date, hist_end_date)
            if not historical_df.empty:
                all_df = pd.concat([all_df, historical_df], ignore_index=True)
...
def main(n_years=5):
    ...
    if os.path.exists(output_path):
        print(f"检测到已有文件 {output_path}，读取最新日期...")
        ...
        if not existing_df.empty and 'Date(日期)' in existing_df.columns:
            # 获取现有数据中的最晚日期
            latest_date = pd.to_datetime(existing_df['Date(日期)']).max()
            latest_date_obj = latest_date.date()
            print(f"现有数据最晚日期为: {latest_date_obj}")

            # 从最晚日期的下一天开始获取新数据
            start_date_obj = latest_date_obj + timedelta(days=1)
            today = datetime.today().date()

            if start_date_obj <= today:
                print(f"获取 {start_date_obj} 至 {today} 的新数据...")

                # 获取从最晚日期到现在的数据
                new_gold_df = get_recent_gold_data_from_date(start_date_obj)
                ...
                if not new_gold_df.empty:
                    ...
                    print(f"新数据已追加到 {output_path}")
                    print(f"新增了 {len(new_gold_df)} 条记录")
            ...
            print("现有文件中没有日期列或数据为空，重新获取全部数据")
            # 按原逻辑获取数据
